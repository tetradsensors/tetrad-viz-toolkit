...
class Animation:
    ...
    def create_animation_from_overlays(
        self,
        overlays_starting_pattern,  # includes path
        overlay_bounds,  # {north:, south:, east:, west:}
        video_path='/tmp/animation/animation',
        fps=20
    ):
        delay = 1

        assert False, "Not Implemented, do not use"
...
def full_animation_run(
    ...
    dt_range = [start + datetime.timedelta(minutes=15 * i)
